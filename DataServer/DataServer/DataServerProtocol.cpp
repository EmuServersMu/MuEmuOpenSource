...
#if SKIN
        pMsg.RentSkin = (DWORD)gQueryManager.GetAsInteger("RentSkin");
#endif
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	gQueryManager.ExecQuery("UPDATE Character SET Kills=%d, Deads=%d, RentSkin=%d WHERE Name='%s'", lpMsg->Kills, lpMsg->Deads, lpMsg->RentSkin, lpMsg->name);
	gQueryManager.Close();

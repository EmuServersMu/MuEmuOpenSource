...
void cFog::Load(char* path)
{
	...
	try
	{
		while(true)
		{
			...
			info.Red = static_cast<float>(lpMemScript->GetAsNumber());

			info.Green = static_cast<float>(lpMemScript->GetAsNumber());

			info.Blue = static_cast<float>(lpMemScript->GetAsNumber());
